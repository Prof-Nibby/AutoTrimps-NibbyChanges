...
function PraidHarder() {
    ...
    if ((game.global.world + maxPlusZones) % 10 > 5)
        maxPlusZones = Math.max(maxPlusZones + (5 - (game.global.world + maxPlusZones) % 10), 10);
...
function trimpcide() {
    if (game.portal.Anticipation.level > 0) {
        ...
        if (game.global.fighting && ((game.jobs.Amalgamator.owned > 0) ? Math.floor((new Date().getTime() - game.global.lastSoldierSentAt) / 1000) : Math.floor(game.global.lastBreedTime / 1000)) >= antistacklimit && (game.global.antiStacks < antistacklimit || antistacklimit == 0 && game.global.antiStacks >= 1))
            forceAbandonTrimps();
...
function autoBoneChargeWhenMax() {
  ...
  if (boneChargesAvailable > 5 && currentZone >= chargeZone) {
    game.permaBoneBonuses.boosts.consume();
